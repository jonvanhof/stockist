...
class league_txns():
    ...
    def pull(self, refresh=False):
        ...
        if self.tx_type is None:
            q_res = self.db.get(self.mdl, q_filter = {'lg_key' : self.s_key +
              ".l." + self.lg_key}, o_by='tx_id.a')
        else:
            q_res = self.db.get(self.mdl, q_filter = {'lg_key' : self.s_key +
              ".l." + self.lg_key, 'tx_type' : self.tx_type}, o_by='tx_id.a')
    
        if len(q_res) > 0:
            if refresh == True:
                self.stock(q_time, restock=True)
                ret_res = self.pull()
            else:
                ret_res = q_res
        else:
            chk = self.stock(q_time)
            if chk is True:
                ret_res = self.pull()
            else:
                ret_res = []
